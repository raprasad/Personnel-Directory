...
add_filter( 'authenticate', 'hu_pin2_auth', 10, 3 );

// based on: http://ben.lobaugh.net/blog/7175/wordpress-replace-built-in-user-authentication#Enough_blabbing_show_me_the_code

function hu_pin2_auth( $user, $username, $password ){
    // Make sure a username and password are present for us to work with
    if($username == '' || $password == '') return;
     
    #$response = wp_remote_get( "/auth_serv.php?user=$username&pass=$password" );
    $response = wp_remote_get( "https://mcbintranet.unix.fas.harvard.edu/auth_serv.php?user=$username&pass=$password" );
