...
// https://mcbintranet.unix.fas.harvard.edu/wp-content/plugins/hu-authz-plugin/authz_checker.php
    ...
    */
    ...
    function show_error(){
        ...
        if ($this->err_url_parse){
            print 'Failed to parse url.  (id: err_url_parse)';
        }
        if ($this->err_no_azp_token){
            print 'AuthZProxy token not found (_authz_token).  (id: err_no_azp_token)';
        }
        if ($this->err_layer1_gnupg_home_directory_not_found){
            print 'GNUPG directory not found.  (id: err_layer1_gnupg_home_directory_not_found)';
        }
        if ($this->err_layer1_decrypt_failed){
            print 'Failed to decrypt url with private key.  (id: err_layer1_decrypt_failed)';
        }
        if ($this->err_layer2_decrypt_failed){
            print 'Failed layer 2 decrypt.  (id: err_layer2_decrypt_failed)';
        }
        if ($this->err_layer2_signature_fail){
            print 'Failed to verify signature with public key.  (id: err_layer2_signature_fail)';
        }
        if ($this->err_layer3_not_two_parts){
            print 'Failed to find authentication and attribute parts of url.  (id: err_layer3_not_two_parts)';
        }
        if ($this->err_layer3_missing_user_vals){
                print 'At least one of the user data attributes was blank.  (id: err_layer3_attribute_data_part_fail)';
            }
        if ($this->err_layer3_attribute_data_part_fail){
            print 'Failed to find all user data attributes in url (email, fname, lname).  (id: err_layer3_attribute_data_part_fail)';
        }
        if ($this->err_layer3_authen_data_part_fail){
            print 'Failed to find all 5 authentication data pieces.  (id: err_layer3_authen_data_part_fail)';
        }
        if ($this->err_layer4_app_name_not_matched){
            print 'Failed to match application name.  (id: err_layer4_app_name_not_matched)';
        }
        if ($this->err_layer4_app_name_not_matched){
            print 'Failed to pass IP address check.  (id: err_layer4_app_name_not_matched)';
        }
        if ($this->err_layer4_token_time_elapsed){
            print "Too much time passed.  Token has elapsed.  (More than $this->expiration_limit_in_seconds seconds have passed).  (id: err_layer4_token_time_elapsed)";
        }
